...
class AddressTable extends React.Component {
    ...
    _getNewAddressId() {
        return this.state.addressList.length
            ? this.state.addressList[this.state.addressList.length - 1].id + 1
            : 1;
    }
    ...
    handleInputChange(addressId, inputFieldName, inputFieldValue) {
        if (addressId) {
            this.setState(prevState => ({
                addressList: prevState.addressList.map(address => {
                    return address.id === addressId
                        ? Object.assign(address, { [inputFieldName]: inputFieldValue })
                        : address;
                }),
            }));
        } else {
            const newAddressField = Object.assign(
